...
class OrderTools(
    ...
    @LLMDescription(
        ...
        戻り値は JSON で、status / orderId / note の各フィールドを含む。
        """,
    )
    fun getOrderStatus(
        ...
        val result = GetOrderStatusResult(
            orderId = orderId,
            status = "processing",
            note = "発送準備中",
        )
        return OrderToolJson.encodeToString(result)
    }
    ...
    @LLMDescription(
        ...
        戻り値は JSON で、status (accepted | already_cancelled) / orderId / cancelledAt の各フィールドを含む。
        実行後はツールの応答 JSON の status フィールドに沿って結果をお客様に伝えること。
        """,
    )
    fun cancelOrder(
