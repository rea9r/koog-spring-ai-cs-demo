...
class SupportGraphService(
    ...
    companion object {
        ...
        private val ANSWER_PROMPT = """
            ...
            # 会話の継続性について（重要）
            このセッションには過去の turn の会話履歴が含まれます。次のルールで扱ってください:
            - 過去 turn であなた（assistant）が発した「お客様」「田中太郎様」のような呼びかけは、
              **架空の例示ではなく、実際にこのセッションでお客様自身を指して使った呼称** です。
              「例示として使ったもの」「実在の方を指すものではない」のような自己否定をしないこと。
            - 「私の名前を覚えてますか?」のような問いには、ChatMemory や LTM 由来の記憶情報をそのまま
              「はい、〇〇様としてお呼びしています」のように肯定的に伝えること。
            - 自分の過去発話を「例示」「サンプル」「仮の表現」と再解釈しないこと。
            ...
            FAQ セクションがない、または FAQ に直接の答えがない場合は、推測で日数・金額・条件・
            業務プロセス・内部運用フローを答えず、「個別にお調べします」「サポートまでご連絡ください」
            のような案内に留めること。配送業者の選定基準や内部システムの仕組みなど、FAQ や注文情報に
            ない運用フローをもっともらしく推測して説明しないこと。
